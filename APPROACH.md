...
- **Field format/validation.** `requiredFieldsFilled()`/`hasEmptyRequiredGuestField()` originally only verified fields held *some* non-empty value, not that the site's own validation rules accepted it - a distinct, deeper question from "was this field even attempted." Live-found on Mews: a proof run reached a genuine payment page with all four guest fields non-empty, but the phone field itself was rejected by the site's own validation ("Select the country code and enter a valid number"). Fixed with `hasInvalidField()` - a generic, site-agnostic scan for any visible `[aria-invalid="true"]` element - wired into the same gate. Live re-verified: the check now correctly blocks a rejected value, and the model self-corrects to a valid one before the run is allowed to succeed. See README's "Mews-specific fixes" section for the full chain of seven real bugs this category surfaced.
